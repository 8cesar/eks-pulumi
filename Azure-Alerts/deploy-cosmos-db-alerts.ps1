...
$templatefileIncDimensions = '\templates\alerts\cosmosDbIncDimensions.json'
...
for ($i = 0; $i -le $arraydata.GetUpperBound(0); $i++) {
    ...
    if ($dimensionsName[$i]) {
        ...
        New-AzResourceGroupDeployment -name $deployment -ResourceGroupName $resourceGroup[$i] -TemplateFile $tempFilePathIncDimensions -alertGroupName $alertGroupName[$i] -alertGroupRegr $alertGroupRegr -cosmoDBName $cosmoDBName[$i] -alertSeverity $alertSeverity[$i] -evaluationFrequency $evaluationFrequency[$i] -windowSize  $windowSize[$i]`
